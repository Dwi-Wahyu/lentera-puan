...
export const CheckupForm: React.FC<CheckupFormProps> = ({
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if (result?.error) {
      ...
      toast.success(
        "Berhasil",
        `Data pemeriksaan berhasil ${initialData ? "diperbarui" : "disimpan"}.`,
      );
    }
  }
    ...
    <Card className="shadow-sm border-t-2 border-t-primary !rounded-2xl">
      <form action={handleSubmit} className="space-y-8">
        ...
        <div className="flex items-center gap-2.5 pb-4 border-b border-outline-variant/50">
          ...
          <h2 className="font-bold text-sm text-on-surface uppercase tracking-widest">
          ...
          >
            {isPending ? (
              ...
              <>
                ...
                {initialData ? "Update Pemeriksaan" : "Simpan Pemeriksaan"}
              </>
            )}
          </Button>
        </div>
