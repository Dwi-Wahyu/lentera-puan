    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-on-surface">Kalender Konseling</h1>
            <p className="text-on-surface-variant">Jadwal pendampingan periode {monthName} {currentYear}.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-7 gap-1">
        {['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'].map(day => (
          <div key={day} className="text-center py-2 text-xs font-bold text-on-surface-variant uppercase tracking-widest border-b border-outline-variant">
            {day}
          </div>
        ))}
          ...
          return (
            <div key={day} className="min-h-[120px] bg-surface-container-lowest border border-outline-variant p-2 group hover:bg-surface-container transition-colors">
              <div className="flex justify-between items-start">
                <span className={`text-sm font-bold ${day === now.getDate() ? 'w-7 h-7 bg-primary text-on-primary rounded-full flex items-center justify-center' : 'text-on-surface'}`}>
                  {day}
                </span>
      ...
      <Card title="Daftar Sesi Bulan Ini" subtitle="Klik pada sesi untuk melihat detail">
         <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-outline-variant">
                  <th className="text-left py-3 text-sm font-bold text-on-surface">Tanggal</th>
                  <th className="text-left py-3 text-sm font-bold text-on-surface">Waktu</th>
                  <th className="text-left py-3 text-sm font-bold text-on-surface">Pendamping</th>
                  ...
                  <tr key={session.id} className="hover:bg-surface-container-low transition-colors group text-sm">
                    ...
                    <td className="py-4">
                       <Link href={`/dashboard/konseling/${session.id}`}>
                          <Button variant="ghost" size="sm" className="p-1 px-2 h-7 text-xs">Detail</Button>
                       </Link>
                    </td>
                  </tr>
                ))}
