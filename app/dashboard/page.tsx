  ...
  return (
    <div className="space-y-7">
      ...
      <div className="flex justify-between items-end gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-on-surface">
            Selamat datang, <span className="text-primary">{session.user?.name?.split(' ')[0]}</span> 👋
          </h1>
        </div>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden group-hover:-translate-y-0.5">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[9px] font-bold text-primary uppercase tracking-widest bg-primary/8 px-2 py-0.5 rounded-full border border-primary/15">KIA</span>
              </div>
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Total Pasien</p>
              <p className="text-3xl font-bold text-primary mt-1 leading-none">{patientCount}</p>
              <div className="flex items-center gap-1 text-secondary mt-2">
                <ArrowUpRight className="w-3 h-3" />
                <span className="text-[10px] font-semibold">Aktif terpantau</span>
              </div>
            </div>
          </div>
        </Link>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden group-hover:-translate-y-0.5">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                {reportCount > 0 && (
                  <span className="text-[9px] font-bold text-error uppercase tracking-widest bg-error/8 px-2 py-0.5 rounded-full border border-error/15">Baru</span>
                )}
              </div>
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Laporan Krisis</p>
              <p className="text-3xl font-bold text-error mt-1 leading-none">{reportCount}</p>
              <span className="text-[10px] text-error/70 font-semibold mt-2 block">Butuh Intervensi Segera</span>
            </div>
          </div>
        </Link>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden group-hover:-translate-y-0.5">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[9px] font-bold text-secondary uppercase tracking-widest bg-secondary/8 px-2 py-0.5 rounded-full border border-secondary/15">Rumah Aman</span>
              </div>
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Kapasitas</p>
              <p className="text-3xl font-bold text-secondary mt-1 leading-none">{occupancyRate}%</p>
              <span className="text-[10px] text-on-surface-variant font-semibold mt-2 block">{availableBeds} Bed Tersedia</span>
            </div>
          </div>
        </Link>
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-5 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden group-hover:-translate-y-0.5">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[9px] font-bold text-primary-container uppercase tracking-widest bg-primary-container/10 px-2 py-0.5 rounded-full border border-primary-container/20">Konseling</span>
              </div>
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Sesi Aktif</p>
              <p className="text-3xl font-bold text-primary-container mt-1 leading-none">{sessionCount}</p>
              <span className="text-[10px] text-on-surface-variant font-semibold mt-2 block">Dijadwalkan</span>
            </div>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card title="Laporan Krisis Terbaru" subtitle="Kasus yang memerlukan perhatian segera">
          <div className="overflow-x-auto -mx-1">
            <table className="w-full">
              <thead>
                <tr className="border-b border-outline-variant/50">
                  <th className="text-left py-2.5 px-2 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">ID Kasus</th>
                  <th className="text-left py-2.5 px-2 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Status</th>
                  <th className="text-left py-2.5 px-2 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Prioritas</th>
                  <th className="text-left py-2.5 px-2 text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Aksi</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/30">
                {recentReports.length > 0 ? recentReports.map((item) => (
                  <tr key={item.id} className="hover:bg-surface-container-low/60 transition-colors group">
                    <td className="py-3 px-2 text-xs font-bold text-primary font-mono">{item.id.slice(-8).toUpperCase()}</td>
                    ...
                    <td className="py-3 px-2">
                      <Link href={`/dashboard/krisis/${item.id}`}>
                        <Button variant="ghost" size="sm" className="gap-1.5 group-hover:bg-primary group-hover:text-on-primary transition-all h-7 px-2">
                  ...
                  <tr>
                    <td colSpan={4} className="py-10 text-center">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm text-on-surface-variant">Belum ada laporan krisis</p>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        <Card title="Aktivitas KIA Terkini" subtitle="Pemantauan kesehatan ibu dan anak">
          <div className="space-y-3">
            {recentActivities.length > 0 ? recentActivities.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg border border-outline-variant/50 hover:border-primary/40 hover:bg-primary/2 transition-all group">
                <div className="flex gap-3 items-center min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    <span className="font-semibold text-on-surface text-sm truncate">{item.patient.name}</span>
                    <span className="text-[10px] text-on-surface-variant truncate">{item.notes || 'Pemeriksaan rutin'}</span>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end gap-1.5 shrink-0 ml-2">
                  <div>
                    <span className="text-xs font-semibold text-primary block">{item.date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                  ...
                  <Link href={`/dashboard/kia/${item.patientId}/checkup/${item.id}`}>
                    <Button variant="ghost" size="sm" className="p-1 px-2 h-6 text-[9px] gap-1">
                      <Eye className="w-3 h-3" /> Detail
                    </Button>
                  </Link>
                </div>
              ...
              <div className="py-10 text-center border border-dashed border-outline-variant/60 rounded-xl">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-sm text-on-surface-variant">Belum ada aktivitas KIA terbaru</p>
                </div>
              </div>
            )}
          </div>
