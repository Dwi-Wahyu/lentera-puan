    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-on-surface">{safeHouse.name}</h1>
            <p className="text-on-surface-variant flex items-center gap-2">
              <Shield className="w-4 h-4 text-primary" />
              Tingkat Keamanan: <span className="font-bold">{formatEnum(safeHouse.safetyLevel)}</span>
            </p>
          </div>
        </div>
        ...
        <Card className="lg:col-span-1 border-t-4 border-t-primary">
          <div className="space-y-6">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-bold text-on-surface-variant uppercase tracking-wider">Status Hunian</p>
                <h3 className="text-4xl font-bold text-primary mt-1">{safeHouse.occupied} / {safeHouse.capacity}</h3>
                <p className="text-xs text-on-surface-variant mt-1 font-medium">Bed Terisi</p>
              </div>
              <div className={`p-2 rounded-lg ${
                safeHouse.status === 'TERSEDIA' ? 'bg-secondary-container text-on-secondary-container' : 'bg-error-container text-on-error-container'
              }`}>
                {safeHouse.status === 'TERSEDIA' ? <CheckCircle2 className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
              </div>
            </div>
            ...
            <OccupancyActions id={id} occupied={safeHouse.occupied} capacity={safeHouse.capacity} />
          </div>
        </Card>

        {/* Location & Contact Info */}
        <Card title="Detail Lokasi" subtitle="Hanya untuk akses internal terbatas" className="lg:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
            <div className="space-y-4">
              <div className="flex gap-3">
                ...
                <div>
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Alamat / Koordinat</label>
                  <p className="text-on-surface font-medium mt-1 leading-relaxed">
                    {safeHouse.location || 'Data lokasi tidak tersedia (Rahasia)'}
                  </p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <label className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Sertifikasi Keamanan</label>
                  <p className="text-on-surface font-medium mt-1">Level {formatEnum(safeHouse.safetyLevel)} - Audit Terakhir 10 Mei 2026</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-surface-container-low p-4 rounded-xl space-y-4 border border-outline-variant">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-on-surface-variant">Penanggung Jawab</span>
                  <span className="text-sm font-bold">Ibu Rahmawati</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-on-surface-variant">Nomor Siaga</span>
                  <span className="text-sm font-bold text-primary">0811-9988-7766</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-on-surface-variant">Polsek Terdekat</span>
                  <span className="text-sm font-bold">Sektor Selatan (2km)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Real Occupants List */}
      <Card title="Daftar Penghuni Saat Ini" subtitle="Data inisial untuk perlindungan privasi">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-outline-variant">
              {safeHouse.residents.length > 0 ? safeHouse.residents.map((resident: any) => (
                ...
                <tr>
                  <td colSpan={5} className="py-8 text-center text-on-surface-variant italic">Belum ada penghuni aktif yang terdaftar di sistem untuk lokasi ini.</td>
                </tr>
              )}
            </tbody>
          </table>
